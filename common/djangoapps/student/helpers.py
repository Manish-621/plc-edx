...
def do_create_account_custom(form, skip_email = False):
    """
    Given cleaned post variables, create the User and UserProfile objects, as well as the
    registration for this user.
    ...
    password = normalize_password(form["password"])
    user.set_password(password)
    registration = Registration()
    ...
    profile = UserProfile(
        user=user,
        **{key: form[key] if key in form.keys() else None for key in profile_fields}
    )
    try:
        profile.save()
    except Exception:
