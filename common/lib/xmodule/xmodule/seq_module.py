...
class SequenceModule(SequenceFields, ProctoringFields, XModule):
    ...
    def _student_or_public_view(self, context, prereq_met, prereq_meta_info, banner_text=None, view=STUDENT_VIEW):
        ...
        IDE['enable'] = False
        CodeEditor = {}
        CodeEditor['enable'] = False
        CodeEditor['coding_submits'] = {}
        question_details = {}
        user = User.objects.filter(pk=self.runtime.user_id).first() 
        viewFile = "seq_module_assessment.html"
        proctored_exam = None
        course_block_tree = None
        ...
        try:                      
            proctored_exam = get_exam_by_content_id(text_type(self.runtime.course_id),text_type(self.location))
        except ProctoredExamNotFoundException:
            pass  
        
        if proctored_exam is not None:
            ide_exam = IDEExams.get_ide_exam(proctored_exam)
            if ide_exam:
                viewFile = "seq_module_ide.html"
                self.get_ide_config(IDE, ide_exam, user)
            elif course_overview.get_course_type() == CourseTypeChoice.Project.value :
                ide_exam = IDEExams.get_ide_handsOn(text_type(self.runtime.course_id))
                if ide_exam:
                    viewFile = "seq_module_ide.html"
                    self.get_ide_config(IDE, ide_exam, user)

                
            if CodingQuestion.has_coding_questions(proctored_exam) :
                CodeEditor['enable'] = True
                viewFile = "seq_module_coding.html"
                question_details['unit_id'] = items[self.position-1]['id'].split('type@vertical+block@')[1]
                question_details['subsection_id'] = proctored_exam['content_id']
                CodeEditor['coding_submits']['saved'], CodeEditor['coding_submits']['submitted'] = CodingResult.get_submitted_questions(proctored_exam,user)

            attempt = ProctoredExamStudentAttempt.objects.get_exam_attempt(proctored_exam['id'],self.runtime.user_id)
            proctored_data = ProctoringData.get_blur_data_summary(attempt)
            if proctored_exam is not None and proctored_exam['is_active']:
                is_timed = True
                exam_id = proctored_exam['id']
                if proctored_exam['hide_after_due']:
                    exam_config = UserProctoringConfiguration.get_user_proctoring_parameters(proctored_exam, user)
                    is_proctored = True
        
        else:
            if course_overview.get_course_type() == CourseTypeChoice.Course.value :
                course_block_tree = get_course_outline_block_tree(
                    context['custom_request'], course_id, user
                )
                viewFile = "seq_module_course.html"
            elif course_overview.get_course_type() == CourseTypeChoice.Project.value :
                ide_exam = IDEExams.get_ide_handsOn(text_type(self.runtime.course_id))
                if ide_exam:
                    viewFile = "seq_module_ide.html"
        ...
        fragment.add_content(self.system.render_template("unit_views/"+viewFile, params))
    ...
    def get_ide_config(self, IDE, ide_exam, user):
        from openedx.features.IDE_Exam.models import IDEExams, IDEExamAttempt
        IDE['enable'] = True
        IDE['is_embedded'] = ide_exam.is_embedded
        IDE['is_token_generated']=  False
        IDE['is_forked']=False
        IDE['ide_url'] = None
        ide_attempt = IDEExamAttempt.get_IDE_Attempt(ide_exam, user)
        if ide_attempt:
            IDE['ide_url'] = ide_attempt.container_url if ide_attempt.container_url else None
            if ide_exam.enable_git_oauth and ide_attempt.git_detais:
                gitdetails=json.loads(ide_attempt.git_detais)
                IDE['is_token_generated']=  gitdetails["is_token_generated"] if 'is_token_generated' in gitdetails else False
                IDE['is_forked']=gitdetails["is_forked"] if 'is_forked' in gitdetails else False
                IDE['git_url']=gitdetails["repository_url"] if 'repository_url' in gitdetails else False
        IDE['username'] = user.username
        IDE['email'] = user.email
        IDE['ide_exam_id'] = ide_exam.id
        IDE['container_grp']= str(ide_exam.proctored_exam.exam_name+'_'+user.username+'_'+str(ide_exam.id)).replace(' ','')
        IDE['enable_github_oauth']= ide_exam.enable_git_oauth
        IDE['client_id'] = settings.GITHUB_CLIENT_ID
        IDE['oauth_login_url'] = settings.GITHUB_LOGIN_URL
        IDE['oauth_redirect_url'] = settings.GITHUB_REDIRECT_URL
    ...
    def _render_student_view_for_items(self, context, display_items, fragment, view=STUDENT_VIEW):
        ...
        for item in display_items:
            ...
            iteminfo = {
                ...
                'graded': item.graded,
                'page_title_parent':display_names[1]
            }
