...
class CourseGrade(CourseGradeBase):
    ...
    #@lazy
    def is_subsection_attempted(self, location):
        """
        Returns an ordered dictionary of subsection grades,
        keyed by subsection location.
        """
        for chapter in self.chapter_grades.itervalues():
            for subsection_grade in chapter['sections']:
                if (str(subsection_grade.location) == location) and subsection_grade.all_total.first_attempted:
                    return True
        return False

    def get_subsection_problemcount(self, location):
        """
        Returns an ordered dictionary of subsection grades,
        keyed by subsection location.
        """
        for chapter in self.chapter_grades.itervalues():
            for subsection_grade in chapter['sections']:
                if (str(subsection_grade.location) == location) :
                    return str(len(subsection_grade.problem_scores))
        return str('N/A')

    def get_subsection_location(self, location):
        """
        Returns an ordered dictionary of subsection grades,
        keyed by subsection location.
        """
