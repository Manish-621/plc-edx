...
from openedx.features.code_editor.models import CodingQuestion, CodingResult
...
def instructor_dashboard_2(request, course_id):
    ...
    section_evaluvation = _section_evaluvation(course, access, course_id)
    if section_evaluvation['isactive'] :
        sections.append(section_evaluvation)
...
def _section_evaluvation(course, access,course_id):
    """ Provide data for the corresponding dashboard section """
    course_key = course.id
    ccx_enabled = settings.FEATURES.get('CUSTOM_COURSES_EDX', False) and course.enable_ccx
    enrollment_role_choices = configuration_helpers.get_value('MANUAL_ENROLLMENT_ROLE_CHOICES',
    ...
    section_data = {
        'section_key': 'evaluvation',
        'section_display_name': _('Evaluation'),
        'access': access,
        'ccx_is_enabled': ccx_enabled,
        'enroll_button_url': reverse('students_update_enrollment', kwargs={'course_id': unicode(course_key)}),
        'unenroll_button_url': reverse('students_update_enrollment', kwargs={'course_id': unicode(course_key)}),
    ...
    coding_exams=CodingQuestion.get_all_questions_course(str(course_key))
    section_data['courseAttended']= coding_exams

    #import ipdb; ipdb.set_trace()

    db=connect_to_mongodb("openedx","mongodb")
    course_key = CourseKey.from_string(course_id)
    usage_id_id = { "org": course_key.org, "course": course_key.course, "run": course_key.run}
    course_idx = db.modulestore.active_versions.find_one(usage_id_id)
    data = db.modulestore.structures.find_one({'_id' : course_idx['versions']['published-branch']})

    titles = {}
    for question in coding_exams:
        unit_block =  filter(lambda x: x['block_id'] == question.unit_id, data['blocks'])
        try:
            titles[question.id]=unit_block[0]['fields']['display_name']
        except :
            titles[question.id] = 'NA'


    if coding_exams :
        coding_exams_ids= [x.proctored_exam_id for x in coding_exams]
        section_data['marks']=coding_exams.values('id','unit_id','max_score','id')
        students_count = {}
        [students_count.update({x.id:CodingResult.get_attempts_count(x)}) for x in coding_exams]
        section_data['students_count'] = students_count
        section_data['isactive']=True
        section_data['titles'] = titles
    else :
        section_data['isactive']=False
    return section_data
...
@login_required
def get_students_on_exams(request):
    user = request.user
    coding_exam_id = request.POST.get('coding_exam_id')
    students=CodingResult.objects.filter(coding_exam_id=coding_exam_id)
    if students.first() :
       course_id=students.first().coding_exam.proctored_exam.course_id
       course_key = CourseKey.from_string(course_id)
       course = get_course_by_id(course_key, depth=0)
       access =  bool(has_access(user, 'staff', course))
       if not access:
        raise Http404()
    context = {
        'students':students,
    }
    rendered = render_to_string('instructor/instructor_dashboard_2/students_list.html', context)
    return JsonResponse({'htmlpage': rendered})

@login_required
def get_evaluation_screen(request, id):
    student_data = CodingResult.objects.filter(id=id).first()
    coding_exams = CodingQuestion.objects.filter(id=student_data.coding_exam_id).first()
    course_id=student_data.coding_exam.proctored_exam.course_id
    course_key = CourseKey.from_string(course_id)
    course = get_course_by_id(course_key, depth=0)
    access =  bool(has_access(request.user, 'staff', course))
    enable_run=None
    unit_header= ""
    html_block_result=None
    html_block=None
    html_data=''
    ...
    try:
        db=connect_to_mongodb("openedx","mongodb")
        #course_id = "course-v1:ppp+cs202+2020_T2"
        block_id = student_data.question_id
        if "NA" not in  block_id  and "block" in block_id :
            block_id= student_data.question_id.split("block@",1)[1]
        block_id = None if  student_data.question_id == "NA" or student_data.question_id == "" else block_id
        unit_block_id =  None if  student_data.unit_id == "NA" or student_data.unit_id == "" else student_data.unit_id
        course_key = CourseKey.from_string(course_id)
        usage_id_id = { "org": course_key.org, "course": course_key.course, "run": course_key.run}
        course_idx = db.modulestore.active_versions.find_one(usage_id_id)
        data = db.modulestore.structures.find_one({'_id' : course_idx['versions']['published-branch']})

        if any(x for x in data['blocks'] if x['block_id'] == block_id) :
            html_block =  filter(lambda x: x['block_id'] == block_id, data['blocks'])
            html_block_result = db.modulestore.definitions.find_one({ '_id' :  html_block[0]['definition']})
            html_data = html_block_result['fields']['data']

        if unit_block_id != None :
            unit_block =  filter(lambda x: x['block_id'] == unit_block_id, data['blocks'])
            unit_header=unit_block[0]['fields']['display_name']
    ...
    evaluvationParameter = None
    graededEvaluvationPar = None

    if student_data.evaluation_details :
        graededEvaluvationPar = student_data.evaluation_details

    if coding_exams.evaluvation_parameters:
        evaluvationParameter = coding_exams.evaluvation_parameters

    if not access:
        raise Http404()
    import requests
    # response = requests.get(settings.GLOT_SNIPPET_URL+"/snippets/"+student_data.snippet_id)
    # data = response.json()
    code_language = student_data.snippet_language
    code_snippet = student_data.snippet_text

    result_snippet = None
    if coding_exams.result_snippet_id and len(coding_exams.result_snippet_id) > 1 :
        result_snippet = coding_exams.result_snippet_id.split(',')[coding_exams.coding_Languages.split(',').index(code_language)]

    code_snippet=code_snippet.replace('\n','\\n')
    enable_run = coding_exams.enable_code_run
    context={
        'language':code_language,
        'filename':student_data.snippet_id,
        'code':code_snippet,
        'id':id,
        'score' : student_data.score,
        'isgraded':student_data.is_graded,
        'maxScore' : coding_exams.max_score,
        'evaluvation_par': evaluvationParameter ,
        'graded_eval_par':graededEvaluvationPar,
        'remarks' : student_data.remarks if student_data.remarks else "",
        'enable_run' : enable_run,
        'html_data' : html_data,
        'unit_header': unit_header,
        'unit_id': coding_exams.unit_id,
        'subsection_id':coding_exams.proctored_exam.content_id,
        'language_type' : coding_exams.coding_Languages_type,
        'result_snippet' : result_snippet
    }
    return render_to_response('instructor/instructor_dashboard_2/CodeEvaluation.html', context)


@login_required
def save_exam_score(request):
   id = request.POST.get('coding_exam_id')
   score = request.POST.get('score')
   #evaluvation_parameter = request.POST.get('evaluvation_parameters')
   evaluvation_parameter = None
   remarks=request.POST.get('remarks')
   if (CodingResult.save_exam_score(id, score,evaluvation_parameter,remarks)) :
