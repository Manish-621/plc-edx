...
from enums import CourseTypeChoice

log = logging.getLogger(__name__)

class CourseCustomDetails(models.Model):
    # User Added fields - Author : Naren
    course_key = TextField(null=True)
    display_name = TextField(null=True)
    Duration = IntegerField(default = 60)
    FocusArea = TextField(default='N/A')
    Tags = TextField(null=True,blank=True)
    course_type_choice = models.CharField(max_length=50, choices = [(tag.name, tag.value) for tag in CourseTypeChoice], default='Course')
    course_instructions = TextField(null=True, blank=True)
    organization_logo_url = TextField(null=True , blank=True)
...
class CourseOverview(TimeStampedModel):
    ...
    # User Added fields - Author : Naren
    is_Assessment = BooleanField(default=True)
    Duration = IntegerField(default = 60)
    FocusArea = TextField(default='C++')
    course_custom_details = models.ForeignKey(CourseCustomDetails,null=True,related_name="course_custom_details",on_delete=models.CASCADE)
    ...
    def _create_or_update(cls, course):
        ...
        # Code to update Course Details 
        course_custom_details = CourseCustomDetails.objects.filter(course_key = course.id)
        if (course_custom_details.exists()):
            course_custom_details = course_custom_details.first()
            course_custom_details.display_name = display_name
        else :
            course_custom_details = CourseCustomDetails()
            course_custom_details.course_key = course.id
            course_custom_details.display_name = display_name
            course_custom_details.Duration = 60
            course_custom_details.FocusArea = 'N/A'
            course_custom_details.course_instructions = '[]'
        course_custom_details.save()
        
        course_overview.course_custom_details = course_custom_details
    ...
    def get_course_type(self):
        """Method to get the coursetype"""
        if self.course_custom_details is None or self.course_custom_details.course_type_choice is None or self.course_custom_details.course_type_choice == '':
            return 'COURSE'
        return CourseTypeChoice[self.course_custom_details.course_type_choice].value

    def get_course_instructions(self):
        """Method to get the coursetype"""
        if self.course_custom_details is None or self.course_custom_details.course_instructions is None or self.course_custom_details.course_instructions == '':
            return '[]'
        return self.course_custom_details.course_instructions
